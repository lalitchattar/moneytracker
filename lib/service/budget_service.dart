...
class BudgetService {
  ...
  Future<List<BudgetModel>> checkBudgetExists() async {
    ...
    var result = await database.rawQuery("SELECT * FROM BUDGET WHERE IS_DELETED = ?", [0]);
  ...
  Future<List<BudgetModel>> getDataForBudgetChart(String fromDate, String toDate, type) async {
    String query =
        """SELECT COUNT(T.FINAL_AMOUNT) AS TOTAL_TRANSACTION, B.BUDGET_AMOUNT, SUM(T.FINAL_AMOUNT) AS EXPENSE, AVG(T.FINAL_AMOUNT) AS AVG_EXPENSE, 
